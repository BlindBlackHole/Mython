...
void Function::Print(ostream& os) {
    os << function.name << std::endl;
}

ObjectHolder Function::Call(const std::vector<ObjectHolder> &actual_args)
{
    try {
        for (size_t i = 0; i < function.formal_params.size() && actual_args.size(); ++i) {
            closure_[function.formal_params[i]] = actual_args[i];
        }

        return function.body->Execute(closure_);
    }
    catch (ObjectHolder& obj) {
        return obj;
    }
