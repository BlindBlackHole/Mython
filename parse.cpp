...
private:
  ...
  Runtime::Closure declared_functions;
  ...
  // Function -> [def id(Params) : Suite]*
  unique_ptr<Ast::Statement> ParseFunctionDefenition() {
      Runtime::Method m;
      ...
      auto [it, inserted] = declared_functions.insert({
        func_name,
        ObjectHolder::Own(Runtime::Function(std::move(m)))
      });

      if (!inserted) {
        throw ParseError("Function " + func_name + " already exists");
      }

      return make_unique<Ast::FunctionDefinition>(it->second);
  }
...
//      }
      ...
      if (id_list.empty()) {
          if (auto it = declared_functions.find(last_name); it != declared_functions.end()) {
          ...
          throw "Function " + last_name + " not defined";
      }
